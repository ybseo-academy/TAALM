## refined original file (adamw.py) of the Torch library for the study of TAALM.

import torch
from torch import Tensor
from types import SimpleNamespace
# from torch.optim.optimizer import (Optimizer,
from .taalm_optimizer import (Optimizer, _use_grad_for_differentiable, _get_value, _dispatch_sqrt,
...
class AdamW(Optimizer):
    ...
    def step(self, grads, params_with_grad, lr0='default', closure=None): # grads 추가함.
        ...
        for group in self.param_groups: # param_groups 가 1개인 경우만 생각함. 여러개면 코드 바꿔야함.
            # params_with_grad = []
            # grads = []
            exp_avgs = []
            exp_avg_sqs = []
            ...
            params0 = adamw(
                self, # state때문에 넣어줌
                params_with_grad, # 밖에서 들어옴
                grads, # 밖에서 들어온 grads가 여기 들어감.
                exp_avgs,
                exp_avg_sqs,
                max_exp_avg_sqs,
                state_steps,
                ...
                beta1=beta1,
                beta2=beta2,
                lr=lr0 if not lr0=='default' else group['lr'], #group["lr"],
...
def adamw(
    ...
    *,
    ...
    func = _single_tensor_adamw_oop   ## out-of-place 내가 개조한것임
...
def _single_tensor_adamw_oop(  # out-of-place
    self,
    params: List[Tensor],
    grads: List[Tensor],
    exp_avgs: List[Tensor],
    ...
    *,
    ...
    for i, param in enumerate(params):
        # state0 = [self.state[param]]
        state0 = list(self.state.values())[i]
        ...
        exp_avg_sq = exp_avg_sq.mul(beta2).addcmul_(grad, grad, value=1 - beta2)

        with torch.no_grad(): # state 안의 exp_avg, exp_avg_sq 를 업데이트해주기
            state0['exp_avg'] = exp_avg
            state0['exp_avg_sq'] = exp_avg_sq
        ...
        if capturable or differentiable:
            ...
            param = param.addcdiv(exp_avg, denom)
            ...
            if amsgrad: #False
                ...
                exp_avg_sq = exp_avg_sq + torch.tensor(1e-8)  ### 

                denom = ((exp_avg_sq.sqrt() + torch.tensor(1e-8).to('cuda') )/ bias_correction2_sqrt).add(eps)
            param = param.addcdiv(exp_avg, denom, value=-step_size)
        
        # Lastly, switch back to complex view
        if amsgrad and torch.is_complex(params[i]): # 이거도 건너뜀
            max_exp_avg_sqs[i] = torch.view_as_complex(max_exp_avg_sqs[i])
